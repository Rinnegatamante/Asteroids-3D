...
while (restart == nil) do
	...
	for i = 1, size do
		...
		if x < 0 or y < 0 or x >= width-5 or y >= height-5 then
			createStar(i)
		else
			Screen.drawPixel(math.floor(x), math.floor(y), white, TOP_SCREEN)
			if Screen.get3DLevel() > 0 then
				Screen.drawPixel(math.floor(x+starfield[i].z), math.floor(y), white, TOP_SCREEN,RIGHT_EYE)
			end
		end
	end
	...
	if Controls.check(pad,KEY_A) and not Controls.check(oldpad,KEY_A) then
		...
		elseif menu_index == 2 then
			...
			if gpu then
				Graphics.init()
				logo_gpu = Graphics.loadImage("/logo.png")
				rtype = "GPU"
			else
	...
	oldpad = pad
	Screen.debugPrint(0,225,"v.0.5",white,TOP_SCREEN)
	if Screen.get3DLevel() > 0 then
		Screen.debugPrint(0,225,"v.0.5",white,TOP_SCREEN,RIGHT_EYE)
	end
	Screen.waitVblankStart()
	Screen.flip()
end
...
while true do
	...
	while i <= #asteroids do
		...
		if not (x < 0 or y < 0 or x >= width - 50 or y >= height - 50) then
			-- Collision triggering
				...
				elseif asteroids[i].z < zPos - 0.5 and asteroids[i].z >= zPos - 2 and ((x >= x2_ship and x2 <= x_ship) or (x >= x_ship and x2 <= x2_ship)) and ((y >= yF_ship and y2 <= yF2_ship) or (y >= yF2_ship and y2 <= yF_ship)) then
					if asteroids[i].color == yellow then
						lose = true
						Timer.pause(aster_time)
						Timer.pause(counter)
			...
			if Screen.get3DLevel() > 0 then	
				Screen.fillRect(math.floor(x+asteroids[i].z),math.floor(x2+asteroids[i].z),y,y2,asteroids[i].color,TOP_SCREEN,RIGHT_EYE)
			end
		end
	...
	-- Draw ship
	if gpu then
		Graphics.drawLine(x_ship,xF_ship,yF_ship,y_ship,green)
		Graphics.drawLine(x2_ship,xF_ship,yF_ship,y_ship,green)
		Graphics.drawLine(x_ship,xF_ship,yF2_ship,y_ship,green)
		Graphics.drawLine(x2_ship,xF_ship,yF2_ship,y_ship,green)
		...
		Screen.drawLine(x_ship,xF_ship,yF_ship,y_ship,green,TOP_SCREEN)
		Screen.drawLine(x2_ship,xF_ship,yF_ship,y_ship,green,TOP_SCREEN)
		Screen.drawLine(x_ship,xF_ship,yF2_ship,y_ship,green,TOP_SCREEN)
		Screen.drawLine(x2_ship,xF_ship,yF2_ship,y_ship,green,TOP_SCREEN)
	...
	if Screen.get3DLevel() > 0 then
		Screen.initBlend(TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x_ship+zPos),math.floor(xF_ship+zPos),yF_ship,y_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x2_ship+zPos),math.floor(xF_ship+zPos),yF_ship,y_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x_ship+zPos),math.floor(xF_ship+zPos),yF2_ship,y_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x2_ship+zPos),math.floor(xF_ship+zPos),yF2_ship,y_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x_ship+zPos),math.floor(x_ship+zPos),yF_ship,yF2_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x_ship+zPos),math.floor(x2_ship+zPos),yF_ship,yF_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x_ship+zPos),math.floor(x2_ship+zPos),yF2_ship,yF2_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.drawLine(math.floor(x2_ship+zPos),math.floor(x2_ship+zPos),yF_ship,yF2_ship,green,TOP_SCREEN,RIGHT_EYE)
		Screen.termBlend()
	end
	
	-- Draw info
	Screen.debugPrint(0,0,"Points: "..((survival_time * 5) + (destroys * 50)),white,BOTTOM_SCREEN)
	Screen.debugPrint(0,30,"Yellow asteroids = Normal asteroids",yellow,BOTTOM_SCREEN)
	Screen.debugPrint(0,45,"Cyan asteroids = Autofire power-up",cyan,BOTTOM_SCREEN)
	Screen.debugPrint(0,60,"Red asteroids = Controls block malus",red,BOTTOM_SCREEN)
	...
	elseif lose then
		...
		Screen.debugPrint(110,125,"Points: "..((survival_time * 5) + (destroys * 50)),white,TOP_SCREEN)
		...
		if Screen.get3DLevel() > 0 then
			...
			Screen.debugPrint(110,125,"Points: "..((survival_time * 5) + (destroys * 50)),white,TOP_SCREEN,RIGHT_EYE)
